...
describe("useActivities", () => {
    ...
    it("adds an activity", () => {
        const { result } = renderHook(() => useActivities())
        act(() => {
            result.current.addActivity({ activity: "Running", duration: 30, completed: false })
        })
    ...
    it("clears all activities", () => {
        ...
        act(() => {
            result.current.addActivity({ activity: "Running", duration: 30, completed: false })
            result.current.addActivity({ activity: "Cycling", duration: 45, completed: false })
        })
        act(() => {
    ...
    it("calculates stats correctly", () => {
        const { result } = renderHook(() => useActivities())
        act(() => {
            result.current.addActivity({ activity: "Running", duration: 30, completed: true })
            result.current.addActivity({ activity: "Cycling", duration: 45, completed: false })
        })
